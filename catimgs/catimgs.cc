...
// first image file
      if(psyimgptr == NULL) {
	psyimgptr=(psyimg *)newinputfile;
	if(outfileclass == psynoclass)outfileclass=infileclass;
// jtlee 04/03/13 changed to default to 1st input file format
        if(orientation == psynoorient) orientation = psyimgptr->getorient();
	if(outtype == psynotype)outtype=intype;
      }
      else {
// cat it onto previous images
	if(inputted_res)
	  concatimgptr=new concatimgs(psyimgptr, (psyimg *)newinputfile, catdim, dim_res);
	else concatimgptr=new concatimgs(psyimgptr, (psyimg *)newinputfile, catdim);
...
// clean up
  ...
  else if(psyimgptr != NULL) delete psyimgptr;
}
